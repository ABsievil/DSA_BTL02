...
class Customer{
	...
	public:
		...
		~Customer(){}
		void print(){ //using for LIMITLESS print inorder
			cout<<result<<endl;
		}
...
class HuffNode {
	...
	public: /* Rotate method */
		...
		HuffNode<E>* Rotation(HuffNode<E>* &root){
			...
			if(key < -1) {     // Right higher
				if(keyR > 0){                            // Right left case
					root->rc = rotateRight(root->rc);
					return rotateLeft(root);
				}
				else return rotateLeft(root);            // Right Right case or right subtree is perfect tree
			}
			else if(key > 1){  // Left higher
				if(keyL < 0){                            // Left right case
					root->lc = rotateLeft(root->lc);
					return rotateRight(root);
				}
				else return rotateRight(root);           // Left Left case or left subtree is perfect tree
			}
			return root;
		}
...
class HuffTree {
    ...
    public:
		...
		string binEncypt(const string& str) {
			...
			if(int(code.length()) > 10) code = code.substr(int(code.length()) - 10);
...
public:
	...
	void calculateFact(unsigned long long int fact[], int N){
		fact[0] = 1;
		for (long long int i = 1; i < N; i++) {
			fact[i] = fact[i - 1] * i;
		}
	}
	int nCr(unsigned long long int fact[], int N, int R){ 
		if (R > N) return 0;
		if(R == N || R == 0) return 1;
		if(fact[R] ==0 || fact[N - R] ==0) return 1;
	...
	int countPermu(vector<int>& arr, unsigned long long int fact[]){
		int N = arr.size();
...
class Operating {
	...
	public:
		...
		void LAPSE(const string& name){
			...
			if(!isGreaterThan3ch) {
				delete ca;
				return ;    //note: name cus must contain from 3 character or more
			}
